# experiments/run_n3_uniform.py
import torch
import numpy as np
import matplotlib.pyplot as plt
import os
from nemot_solver.solver import MOT_MC_Solver
from nemot_solver.data_utils import u_exact_n3

def run(device):
    """Runs the 1D Uniform Density experiment for N=3."""
    
    params = {
        'name': 'N=3 Uniform 1D', 'N_electrons': 3, 'particle_dim': 1,
    ...
    u_exact = u_exact_n3(x_plot.squeeze().numpy())
    ...
    print(f"N=3 Relative L2 Error: {rel_l2_error:.4f}")

    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(10, 7))
    ax.plot(x_plot.numpy(), u_exact_shifted, 'r--', linewidth=4, label='Analytical (N=3)')
    ax.plot(x_plot.numpy(), u_nemot_shifted, 'b-', linewidth=2.5, label='Learned (N=3)')
    ax.set_title('Kantorovich Potential for N=3, d=1 (Shifted)', fontsize=16)
    ax.set_xlabel('Position x', fontsize=12)
    ax.set_ylabel('u(x)', fontsize=12)
    ax.legend(fontsize=12)
    
    if not os.path.exists('plots'): os.makedirs('plots')
    plt.savefig('plots/experiment_n3_uniform.png')
    print("Plot saved to plots/experiment_n3_uniform.png")
    plt.show()