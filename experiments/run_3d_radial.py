# experiments/run_3d_radial.py
import torch
import numpy as np
import matplotlib.pyplot as plt
import os
from nemot_solver.solver import MOT_Symmetric_Potential_Alg_Efficient
from nemot_solver.data_utils import generate_uniform_ball_samples, a_r_exact_ball_3d, derive_f_learned_nd, create_regular_ball_grid

def run(device):
    """Runs the 3D Radially Symmetric experiment with smoothing."""
    
    params = {
        'name': 'Uniform 3D Ball (N=2, d=3)', 'N_electrons': 2, 'particle_dim': 3,
        'epochs': 1000, 'batch_size_support': 512, 'eps_regularization': 0.002,
        'lr': 5e-5, 'K_neurons_phi': 512, 'deeper_phi': True, 'output_scale_phi': 5.0
    }

    dataset = generate_uniform_ball_samples(100000, radius=1.0)
    solver = MOT_Symmetric_Potential_Alg_Efficient(params, device)
    solver.train_potential_network(dataset)

    print("\nGenerating plots for 3D radial case...")
    fig, axes = plt.subplots(2, 2, figsize=(10, 10))
    ((ax_tl, ax_tr), (ax_bl, ax_br)) = axes
    fig.suptitle('Results for N=2, d=3 Uniform Density (Unit Ball)', fontsize=16)
    fig.tight_layout(pad=4.0)

    # --- Top-Left: Kantorovich Potential ---
    solver.potential_model.eval()
    r_plot = torch.linspace(0, 1, 100).to(device)
    points_on_axis = torch.stack([r_plot, torch.zeros_like(r_plot), torch.zeros_like(r_plot)], dim=1)
    with torch.no_grad():
        v_r = solver.potential_model(points_on_axis).cpu()
    ax_tl.plot(r_plot.cpu(), -v_r, color='black')
    ax_tl.set_title('Kantorovich Potential'); ax_tl.set_xlabel('r'); ax_tl.set_ylabel('-v(r)')

    # --- Top-Right: Co-motion function (with smoothing) ---
    source_points = dataset[torch.randperm(len(dataset))[:500]]
    print("Creating regular evaluation grid for smoother results...")
    evaluation_grid = create_regular_ball_grid(resolution=60)
    print(f"Grid created with {len(evaluation_grid)} points.")
    dest_points = derive_f_learned_nd(solver.potential_model, source_points, evaluation_grid.to(device), device)
    ...
    analytical_a_r = a_r_exact_ball_3d(analytical_r)
    ...
    ax_tr.set_title('Radial Co-motion Function a(r)'); ax_tr.set_xlabel('Source Radius r')
    ax_tr.set_ylabel('Destination Radius a(r)'); ax_tr.set_xlim(0, 1); ax_tr.set_ylim(0, 1)
    ...
    plt.savefig('plots/experiment_3d_radial.png')
    print("Plot saved to plots/experiment_3d_radial.png")
    plt.show()
