# experiments/run_1d_triangular.py
import torch
import numpy as np
import matplotlib.pyplot as plt
import os
from nemot_solver.solver import MOT_Symmetric_Potential_Alg_Efficient
from nemot_solver.data_utils import generate_triangular_1d_samples, f_exact_triangular_1d, derive_f_learned_nd

def run(device):
    """Runs the 1D Triangular Density experiment."""
    
    config = {
        'title': "Experiment for 1D Triangular Density",
        'savename': "plots/experiment_1d_triangular.png",
        'params': {
            'name': "Triangular 1D", 'N_electrons': 2, 'particle_dim': 1, 'epochs': 800,
    ...
    dataset = generate_triangular_1d_samples(100000, a=config['a'])
    solver = MOT_Symmetric_Potential_Alg_Efficient(config['params'], device)
    solver.train_potential_network(dataset)

    # --- Plotting ---
    # The plotting logic is identical to the uniform case, just with different data.
    # For a real-world app, you might abstract this plotting logic into a shared function.
    ...
    f_exact = f_exact_triangular_1d(x_plot_np, a=config['a'])
